  ...
  app.get ('/cities', async (req, res) =>
{
  try {
    (async() =>
        res.send(JSON.stringify(await dbApi.cities()))
    )();
  } catch (err)
  {
  ...
  const message =
    {
      ...
      subject: `Reset Password`,
      text: `Hello,
      Please click the link below to reset your password. Link expires in 24 hours. ${req.protocol}://${req.headers.host}/ResetPwd?emailToken=${jwtToken.accessToken}`,
      html: `<h1>Hello,</h1>
      <p>Please click the link below to reset your password. Link expires in 24 hours.</p>
             <a href="${req.protocol}://${req.headers.host}/ResetPwd?emailToken=${jwtToken.accessToken}">Reset your password</a>`
    };
